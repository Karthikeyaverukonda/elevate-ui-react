...
import { Users, Trophy, Star, Building2, Activity, UserCheck, Zap, LogOut, Pencil, ArrowLeft, Moon, Sun } from "lucide-react";
import { useTheme } from "@/hooks/use-theme";

const BASE_URL = "http://127.0.0.1:8000";

const ChampionCard = ({ name, image, nominations, award, theme, subtitle, ringColor = 'ring-yellow-300', badgeDark = 'bg-yellow-900/40 text-yellow-300', badgeLight = 'bg-yellow-100 text-yellow-700' }: { name: string; image: string; nominations: number; award: string; theme: 'light' | 'dark'; subtitle?: string; ringColor?: string; badgeDark?: string; badgeLight?: string }) => (
  <div className={`flex items-center gap-3 p-3 rounded-xl backdrop-blur border transition-all ${
    theme === 'dark'
      ? 'bg-slate-600/30 border-slate-500/30 hover:bg-slate-600/40'
      : 'bg-white/60 border-white/40 hover:bg-white/80'
  } shadow-sm hover:shadow-md`}>
    <img src={image ? BASE_URL + image : "/placeholder.svg"} alt={name} className={`h-10 w-10 rounded-full object-cover ring-2 ${ringColor}`} />
    <div className="flex-1 min-w-0">
      <p className={`font-semibold ${theme === 'dark' ? 'text-white' : 'text-slate-800'} text-sm truncate`}>{name}</p>
      <p className={`text-xs ${theme === 'dark' ? 'text-slate-400' : 'text-muted-foreground'} truncate`}>{subtitle ? `${subtitle} · ${award}` : award}</p>
    </div>
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${theme === 'dark' 
      ...
      <div className={`${theme === 'dark' 
        ...
        : 'bg-white/80'} backdrop-blur border-b ${theme === 'dark' ? 'border-slate-700' : 'border-slate-200'} px-8 py-5 flex items-center justify-between`}>
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className={`text-sm ${theme === 'dark' ? 'text-slate-400' : 'text-muted-foreground'} mt-0.5`}>Here's what's happening across your organization</p>
          </div>
        </div>
        {profile && (
          <div className="flex items-center gap-3">
            ...
            <div className="text-right">
              <p className={`text-sm font-semibold ${theme === 'dark' ? 'text-white' : 'text-slate-800'}`}>{profile.employee_name}</p>
              <p className={`text-xs ${theme === 'dark' ? 'text-slate-400' : 'text-muted-foreground'}`}>{profile.employee_role}</p>
            </div>
            <Button variant="outline" size="sm" onClick={handleOpenEditDialog} className={`ml-2 ${theme === 'dark' 
              ? 'text-slate-300 hover:text-indigo-400 hover:border-indigo-400 border-slate-600 bg-slate-700' 
      ...
      <div className={`px-8 py-6 space-y-8 max-w-7xl mx-auto`}>
        ...
        {profile && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <Card className={`md:col-span-2 ${theme === 'dark'
              ? 'bg-gradient-to-br from-indigo-600 to-purple-700 text-white border-indigo-500/30'
              : 'bg-gradient-to-br from-indigo-500 to-purple-600 text-white'} border-0 shadow-lg hover:shadow-xl transition-shadow`}>
              <CardContent className="p-6 flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-purple-100 text-sm">{profile.employee_role}</p>
                  <div className="flex gap-2 mt-2 flex-wrap">
                    {profile.team_name
                      ? <span className="bg-white/20 text-white text-xs px-2 py-0.5 rounded-full font-medium">{profile.team_name}</span>
                      : <button onClick={handleOpenJoinDialog} className="flex items-center gap-1 bg-white/20 hover:bg-white/30 transition-colors text-white text-xs px-3 py-1 rounded-full font-medium"><Users className="h-3 w-3" />Join a Team</button>
                    }
                    {profile.art_name && <span className="bg-white/20 text-white text-xs px-2 py-0.5 rounded-full font-medium">{profile.art_name}</span>}
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${profile.active_status === "approved" ? "bg-green-400/30 text-green-100" : "bg-yellow-400/30 text-yellow-100"}`}>{profile.active_status}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className={`${theme === 'dark'
              ? 'bg-gradient-to-br from-amber-600 to-orange-600 text-white border-amber-500/30'
              : 'bg-gradient-to-br from-amber-400 to-orange-500 text-white'} border-0 shadow-lg hover:shadow-xl transition-shadow`}>
              <CardContent className="p-6 flex flex-col justify-between h-full">
                <Trophy className="h-7 w-7 text-white/80" />
                <div>
                  <p className="text-3xl font-extrabold">{profile.total_awards}</p>
                  <p className="text-orange-100 text-sm mt-1 font-medium">Awards Earned</p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className={`${theme === 'dark'
              ...
              : 'bg-gradient-to-br from-emerald-400 to-teal-600 text-white'} border-0 shadow-lg hover:shadow-xl transition-shadow`}>
              <CardContent className="p-6 flex flex-col justify-between h-full">
                <Zap className="h-7 w-7 text-white/80" />
                <div>
                  <p className="text-3xl font-extrabold">{profile.total_points}</p>
                  <p className="text-emerald-100 text-sm mt-1 font-medium">Total Points</p>
                </div>
              </CardContent>
            </Card>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Card onClick={() => navigate("/nominations")} className={`border-0 shadow-md cursor-pointer hover:shadow-xl transition-all hover:scale-105 ${theme === 'dark'
            ? 'bg-gradient-to-br from-amber-600 to-yellow-600 text-white border-amber-500/30'
            ...
            <CardContent className="p-6 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-yellow-100 text-sm mt-0.5">Recognise someone who made a difference</p>
              </div>
            </CardContent>
          </Card>

          <Card onClick={() => navigate("/leaderboard")} className={`border-0 shadow-md cursor-pointer hover:shadow-xl transition-all hover:scale-105 ${theme === 'dark'
            ...
            <CardContent className="p-6 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sky-100 text-sm mt-0.5">See who's leading the pack this sprint</p>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        {homePageData && (
          <>
            {/* Pulse stats */}
            <div>
              <h2 className={`text-xl font-bold ${theme === 'dark' ? 'text-white' : 'text-slate-800'} mb-4 flex items-center gap-2`}><Activity className="h-5 w-5 text-indigo-500" />Organisation Pulse</h2>
              <div className="grid grid-cols-2 gap-4">
                <Card className={`border-0 shadow-sm ${theme === 'dark' 
                  ? 'bg-slate-700/50 text-slate-100 border-slate-600' 
                  : 'bg-white'}`}>
                  <CardContent className="p-5 flex items-center gap-4">
                    <div className={`h-12 w-12 rounded-full flex items-center justify-center ${theme === 'dark'
                      ? 'bg-blue-900/40 text-blue-400'
                      : 'bg-blue-100 text-blue-600'
                    }`}><Zap className="h-6 w-6" /></div>
                    <div>
                      <p className={`text-2xl font-extrabold ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{homePageData.total_nominations_done_in_last_day}</p>
                      <p className={`text-sm ${theme === 'dark' ? 'text-slate-400' : 'text-muted-foreground'}`}>Nominations in Last 24h</p>
                    </div>
                  </CardContent>
                </Card>
                <Card className={`border-0 shadow-sm ${theme === 'dark' 
                  ...
                  <CardContent className="p-5 flex items-center gap-4">
                    ...
                    }`}><UserCheck className="h-6 w-6" /></div>
                    <div>
                      <p className={`text-2xl font-extrabold ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{homePageData.total_active_Employees}</p>
                      <p className={`text-sm ${theme === 'dark' ? 'text-slate-400' : 'text-muted-foreground'}`}>Active Employees</p>
                    </div>
                  </CardContent>
                </Card>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <Card className={`border-0 shadow-md ${theme === 'dark'
                ...
                <CardHeader className="pb-2">
                  <CardTitle className={`text-base flex items-center gap-2 ${theme === 'dark' ? 'text-yellow-400' : 'text-yellow-700'}`}>
                ...
                <CardContent className="space-y-2">
                  {homePageData.last_sprint_top5_champions_in_your_art.length > 0
                    ? homePageData.last_sprint_top5_champions_in_your_art.map((c, i) => (
                      <ChampionCard key={i} name={c.employee_name} image={c.employee_image} nominations={c.no_of_nominations_received} award={c.most_received_award_name} theme={theme} />
                    ))
              ...
              <Card className={`border-0 shadow-md ${theme === 'dark'
                ...
                <CardHeader className="pb-2">
                  <CardTitle className={`text-base flex items-center gap-2 ${theme === 'dark' ? 'text-indigo-400' : 'text-indigo-700'}`}>
              ...
              <Card className={`border-0 shadow-md ${theme === 'dark'
                ...
                <CardHeader className="pb-2">
                  <CardTitle className={`text-base flex items-center gap-2 ${theme === 'dark' ? 'text-emerald-400' : 'text-emerald-700'}`}>
                  ...
                  {homePageData.organization_level_champions_top5_till_now.length > 0
                    ? homePageData.organization_level_champions_top5_till_now.map((c, i) => (
                      <ChampionCard
                        ...
                        image={c.employee_image}
                        nominations={c.no_of_nominations_received}
                        award={c.most_received_award_name}
                        subtitle={`${c.team_name} · ${c.art_name}`}
                        theme={theme}
      ...
      <Dialog open={showJoinDialog} onOpenChange={setShowJoinDialog}>
        <DialogContent className={`sm:max-w-md border ${theme === 'dark' ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white text-slate-900'}`}>
          <DialogHeader>
          ...
          <div className="space-y-4 py-2">
            <div>
              ...
              >
                ...
                <SelectContent className={`${theme === 'dark' ? 'bg-slate-800 text-white' : ''}`}>
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent className={`max-w-md border ${theme === 'dark' ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white text-slate-900'}`}>
          <DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              <div className="w-full">
                <Label htmlFor="image" className={`text-sm font-medium ${theme === 'dark' ? 'text-slate-200' : ''}`}>
                  Profile Image
                </Label>
                <Input
                  id="image"
                  ...
                  className={`mt-1 ${theme === 'dark' ? 'text-white' : ''}`}
                />
              </div>
            </div>
            ...
            {/* Login */}
            <div>
              <Label htmlFor="login" className={`text-sm font-medium ${theme === 'dark' ? 'text-slate-200' : ''}`}>
                Login
              </Label>
              <Input
                id="login"
                ...
                className={`mt-1 ${theme === 'dark' ? 'bg-slate-700 border-slate-600 text-white placeholder-slate-400 focus-visible:ring-indigo-500' : ''}`}
              />
            </div>

            {/* First Name */}
            <div>
              <Label htmlFor="firstname" className={`text-sm font-medium ${theme === 'dark' ? 'text-slate-200' : ''}`}>
                First Name
              </Label>
              <Input
                id="firstname"
            ...
            {/* Last Name */}
            <div>
              <Label htmlFor="lastname" className={`text-sm font-medium ${theme === 'dark' ? 'text-slate-200' : ''}`}>
                Last Name
              </Label>
              <Input
                id="lastname"
            ...
            {/* Role */}
            <div>
              <Label htmlFor="role" className={`text-sm font-medium ${theme === 'dark' ? 'text-slate-200' : ''}`}>
                Role
              </Label>
              <select
                id="role"
                ...
                className={`w-full mt-1 px-3 py-2 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 ${theme === 'dark' ? 'bg-slate-700 border-slate-600 text-white' : 'border-slate-300'}`}
              >
            ...
            {/* Password */}
            <div>
              <Label htmlFor="password" className={`text-sm font-medium ${theme === 'dark' ? 'text-slate-200' : ''}`}>
                Password (Optional)
              </Label>
              <Input
                id="password"
